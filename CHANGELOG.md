...
### V2.2 Release（Cycle 11I）— 先天八卦品牌资产统一
- 版本升级为 `versionCode 4` / `versionName 2.2`。
- 桌面 adaptive 图标与罗盘天池中央统一引用 `bagua_core` 矢量资产，品牌图位固定为先天八卦：乾上、坤下。
- 罗盘动态外圈继续使用后天八卦业务方位；八个卦均由真实阴阳爻线绘制，不再依赖 Unicode 字体或重复模板。
- 同步单色层、五档密度 PNG、SVG 源稿、资产规范与资产回归测试；修正中央太极为标准阴阳结构。
...
- 修复 GitHub Actions Ubuntu runner 的 Gradle wrapper 执行权限，并将 CI Fixture 测试时区改为显式注入；最终 Actions run `33853300999` 全绿。
...
- readiness 满足后自动显示“姿态正确，已自动通过”，定盘按钮无需额外确认即可启用；系统减少动画时自动降级为静态提示。
...
#### 验收结论
- 全量单测 76/76；Lint 0 error；Debug/Release 构建通过（release 未签名）
- 完整用户闭环 E2E 全绿：冷启动 1.6s、后台恢复、旋转锁定、持久化跨强杀、AI 降级、0 Crash/ANR
- 罗盘页性能：305 帧/5s，Janky 0.33%
- 真机磁场验收清单化：DOCS/REAL_DEVICE_TEST.md（18 项，未验证不宣称）
